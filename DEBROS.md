...
**Rule (multi-agent surface isolation):** When more than one agent may run in the same cmux instance, each agent MUST keep its terminal work in its own surface, named for the task. An agent launched inside cmux already owns the surface it started in (`$CMUX_SURFACE_ID`); when it needs another (e.g. a long-running dev server *and* a test run), it creates one with `cmux new-surface` and names it via `cmux rename-tab` with a short task label — `build-web`, `e2e-smoke`, `migrate-db`. An agent MUST NOT run work in, or `cmux send` into, a surface another agent owns — shared surfaces interleave output and let one agent kill or misread another's process. Before sending to any surface it did not create, the agent checks `cmux tree` / `cmux list-pane-surfaces` to confirm it's idle and unclaimed.
